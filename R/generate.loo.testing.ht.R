...
generate.loo.testing.ht = function(loo.testing.list,
  ...
  generate.loo.row = function(loo.ind) {
    ...
    loo.row.dt = data.table(
      ...
      `Changepoint model weight` = loo.test$loo.compare[model == 'model1', weight],
      `Null model weight` = loo.test$loo.compare[model == 'model2', weight]
    )
  ...
  loo.testing.ht = huxtable::huxtable(loo.testing.dt) %>%
    set_number_format(everywhere, 2:4, 3) %>%
